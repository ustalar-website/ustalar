...
export default function Ecom() {
    ...
    async (loadMore = false) => {
      ...
      try {
        if (sidebarFilters.subcategories.length > 0) {
          const promises = sidebarFilters.subcategories.map(async (subcategoryId) => {
            const response = await fetch(`https://api.peshekar.online/api/v1/service/${subcategoryId}/professionals/`)
            if (!response.ok) {
              throw new Error(`HTTP error! status: ${response.status} for subcategory ${subcategoryId}`)
            }
          ...
          const currentSortOption = sortOptions.find((option) => option.label === selectedSort)
          if (currentSortOption) {
            if (currentSortOption.value === "top_rated") {
              finalProfessionals.sort((a, b) => (b.average_rating || 0) - (a.average_rating || 0))
            } else if (currentSortOption.value === "newest") {
              finalProfessionals.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
            }
          }
          ...
          let baseUrl = "https://api.peshekar.online/api/v1/professionals/search"
  ...
  }, [selectedSort, sidebarFilters, categoryMap, serviceMap, cityMap, cityNameMap])
    ...
    if (nextPageUrl && sidebarFilters.subcategories.length === 0) {
      ...
      fetch(nextPageUrl)
        ...
        .then((data) => {
          ...
          setTotalProfessionals((prevTotal) => prevTotal + filteredResults.length)
        })
        .catch((err) => setError(err.message))
        .finally(() => setLoading(false))
    } else if (sidebarFilters.subcategories.length > 0) {
      console.warn("Pagination is not supported when multiple subcategories are selected (frontend OR logic).")
    }
  }
        ...
        <div className="ecom-right flex flex-col flex-1">
          <div className="ecom-right-top mb-14">
            <div className="flex justify-between items-center mb-4">
              <h1 className="font-bold text-3xl flex items-center gap-5">
                Peşəkarlar
                <span className="text-xl text-[#525B6B] font-semibold">axtarış nəticələri ({totalProfessionals})</span>
              </h1>
                ...
                <div className="flex justify-end items-center w-full h-full">
                  ...
                  >
                    <div className="flex flex-col items-start pr-8">
                      ...
                      <span className="block text-xs text-gray-600 font-normal mt-2">{selectedSort}</span>
                    </div>
                    ...
                    {sortOptions.map((option) => (
                      <button
                        ...
                        className={`block w-full text-left px-4 py-2 text-sm hover:bg-gray-100 ${
                          selectedSort === option.label ? "bg-blue-600 text-white" : "text-gray-700"
                        }`}
                      >
                        {option.label}
                      </button>
          ...
          <div className="flex flex-col gap-14">
            {loading && professionals.length === 0 ? (
              <div className="text-center text-lg text-gray-600">Peşəkarlar yüklənir...</div>
            ) : error ? (
              <div className="text-center text-lg text-red-500">Xəta: {error}. Zəhmət olmasa, yenidən cəhd edin.</div>
            ) : professionals.length === 0 ? (
              <div className="text-center text-lg text-gray-600">Heç bir peşəkar tapılmadı.</div>
            ) : (
                ...
                {professionals.map((professional) => {
                  ...
                  return (
                    <EcomCard
                      ...
                      specialityName={serviceMap[professional.profession_speciality]}
