            ...
            <div className="hidden sm:flex items-center space-x-4 p-2 ">
              {navItems.map(({ label, to }) => (
                <NavLink
                  ...
                  to={to}
                  className={({ isActive }) =>
                    `${base} ${isActive ? "bg-cyan-900 text-white px-3 py-5 text-sm font-semibold border rounded-xl" : "text-white bg-cyan-900 px-6 py-3 text-sm border font-semibold rounded-xl"}`
                  }
                >
                  {label}
                </NavLink>
          ...
          <div className="space-y-1 px-2 pt-2 pb-3">
            {navItems.map(({ label, to }) => (
              <NavLink
                key={to}
                to={to}
                onClick={() => setIsOpen(false)} // link seçiləndə menyunu bağla
                className={({ isActive }) =>
                  `${mobileBase} ${
